...
expect(/\.cluster-bg\s*\{/.test(style), 'missing .cluster-bg');
expect(/\.cluster-border\s*\{/.test(style), 'missing .cluster-border');
...
expect(/\.cluster-bg\s*\{[\s\S]*fill:\s*#FFFBE6;/.test(style), 'clusterBkg not applied');
expect(/\.cluster-border\s*\{[\s\S]*stroke:\s*#AAAA33;/.test(style), 'clusterBorder not applied');
