...
export function blockOverlay(x: number, y: number, width: number, height: number,
  ...
  for (const br of branchYs) {
    ...
    if (br.title) {
      ...
      parts.push(`<rect class="cluster-title-bg" x="6" y="${yRel - 10}" width="${bw}" height="18" rx="3"/>`);
      parts.push(`<text class="cluster-label-text" x="${6 + bw/2}" y="${yRel + 1}" text-anchor="middle">${escapeXml(br.title)}</text>`);
    }
  }
  parts.push('</g>');
  return parts.join('\n');
